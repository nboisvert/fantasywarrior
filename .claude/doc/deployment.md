...
| *Period calendar* fails every night, *Nightly/Player/Draft/News sync* all show `skipped` | No `Seasons` row for the season `period-init`'s `--season`-less default resolves to. `Season.CurrentOn`'s September cutover starts guessing next season (`FromStartYear(today.Month >= 9 ? today.Year : today.Year - 1)`) the moment the calendar turns September — **before** anyone has necessarily run `season-init` for it, since nothing else forces that to happen in step. Real incident: dead every night from 2026-09-06 to 2026-09-25 for exactly this reason. Not a cold start — `db-migrate` and `stats-sync`, both earlier in the chain and both hitting the same database, succeeded every one of those nights | `season-init --season <next> --start <date> --end <date>` once, well before the next September (a good chunk of the off-season prep checklist, not just something to remember on the day). After that, catch up whatever the chain skipped by hand: `player-sync`, `draft-sync`, `news-sync` at least |
