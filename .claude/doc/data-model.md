...
cascade), AsOfDate, Rank, LastNightPoints, LastNightGamesPlayed, CreatedUtc →
unique (TeamId, AsOfDate) — the upsert key, a rerun of the nightly job for the
same night updates rather than duplicates; index (TeamId, AsOfDate DESC)
INCLUDE (Rank, LastNightPoints) for "this team's two most recent nights".
Written once a night by `StandingsSnapshotJob` (nightly's own step [4/4]),
read back by the Standings screen's Last Night columns and rank-movement pill.
...
| `vTeamPeriodScores` | active/bench points and active games played per team per week → the weekly history |
