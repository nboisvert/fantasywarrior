...
| … | ? | **Trou dans le journal, comme la ligne « 3-11 » plus haut.** `project_status.md` affirme un état plus avancé sous le code `TKW6UR` (2025-11-17, semaine 7, semaines 1-6 banquées) — mais aucune avance n'a été journalisée ici depuis le redémarrage ci-dessus, et ces chiffres correspondent exactement à la ligne 2025-11-17 du **monde `P7R9CT` précédent** (semaine 7, W3-W6 banquées), pas au monde `TKW6UR` qui vient tout juste d'être relancé à la semaine 2. `project_status.md` n'a vraisemblablement pas été remis à jour après ce redémarrage. **Lancer `sim-clock` avant de faire confiance à l'un des deux** ; corriger celui qui est faux à ce moment-là plutôt que de deviner ici. |
