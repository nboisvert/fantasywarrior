...
### Slots
Configurable by the commissioner, per position (`lineup.slots`); Les Mordus' own numbers are in [mordus.md](mordus.md). This is the **only rule enforced at lineup submission**. Fielding **fewer** than the maximum is allowed — you simply score less. Fielding **more** is refused.
...
> **No `recompute` job exists.** See [deployment.md](deployment.md) for un-banking and replaying a week, which is the way back in.
...
**The whole catalogue is [league-rules.md](league-rules.md)** — every parameter, its default, and where it is enforced. What matters to scoring:

| Setting | Where | Enforced? |
|---|---|---|
| Point values | `scoring.values` | yes, at scoring |
| Active slots per position | `lineup.slots` | **yes, at lineup submission** |
| Roster size min/max | `roster.min` / `.max` | **yes, on trades** (proposal and acceptance) |
| Salary cap | `cap.max` | **yes, on trades** (proposal and acceptance) |
| Cost of a player with no contract | `cap.defaultCapHit` | **yes**, in `vStandings` (today's *and* committed columns) and in trade validation |
| Équipe slot value | `scoring.values` (`teamWins`/`teamLosses`/`teamOtLosses`) | yes, at scoring |
| Draft picks per team per year | `draft.rookieRounds` | one per round, generated by `draft-picks-init` |

They live on the `LeagueSeason` being played, read through `RuleSetResolver.ForScoringAsync` — so a scale set for next season does not price this season's week, and a finished season keeps the rules it was played under.
...
All of this is set through the API — `PATCH /api/leagues/{code}/rules`, commissioner only — and the **League rules** panel in the app, which writes the season being *prepared*. A rule change therefore never restates a finished season.
