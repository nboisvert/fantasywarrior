...
- App relies on a **news service** (added 2026-07-28) that pulls NHL news from external RSS feeds — first two sources **Rotowire** and **FantasySP** — into a global `news` Firestore collection (`NewsSyncJob`, run nightly in `daily-jobs.yml`). Not league-scoped: the global ticker (`NewsTicker.tsx`, mounted app-wide) shows these news items unfiltered alongside league trades. Roster-move (add/drop) items were removed from the ticker in the same change — trades + real news only now.
